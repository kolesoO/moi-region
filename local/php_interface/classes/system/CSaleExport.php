...
use CModule;
use CSaleExport as BaseCSaleExport;
...
class CSaleExport extends BaseCSaleExport
{
    ...
    public static function ExportOrders2Xml(
        ...
        $currency = "", $crmMode = false,
        $time_limit = 0, $version = false,
        $arOptions = Array()
    ) {
        $lastOrderPrefix = '';
        $arCharSets = array();
        ...
        <<?=BaseCSaleExport::getTagName("SALE_EXPORT_COM_INFORMATION")?> <?=self::getCmrXmlRootNameParams()?>><?
        ...
        while($arOrder = $dbOrderList->Fetch())
        {
            ...
            $arOrderTax = BaseCSaleExport::getOrderTax($order);
            ...
            foreach($basketItems['result'] as $basketItem)
            {
                ...
                {
                    $r = new Result();
                    $r->addWarning(new Error(GetMessage("SALE_EXPORT_REASON_MARKED_BASKET_PROPERTY").'1C_Exchange:Order.export.basket.properties', 'SALE_EXPORT_REASON_MARKED_BASKET_PROPERTY'));
            ...
            {
                ob_start();
                echo '<'.BaseCSaleExport::getTagName("SALE_EXPORT_CONTAINER").'>';
            }
        ...
        </<?=BaseCSaleExport::getTagName("SALE_EXPORT_COM_INFORMATION")?>><?
